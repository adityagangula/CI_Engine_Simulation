...
RPM = np.arange(800,6000,100)
IHPar = np.zeros(len(RPM))
ITar = np.zeros(len(RPM))
mair = np.zeros(len(RPM))
...
for rpm in RPM:
    ...
    Vd=(pi*(Bore**2)*Rad)/2 # ~367cc
    Vcl=Vd/(r-1)
    #S=(Vd*4)/((Bore**2)*pi)

    N=int(rpm) #engine rpm
    ...
    IHP = IP/745.7
    IHPar[int((N-RPM[0])/100)] = IHP
    IT = IP/(120*pi*N)
    ITar[int((N-RPM[0])/100)] = IT
    mair[int((N-RPM[0])/100)] = mass_of_air_inducted
plt.plot(RPM,IHPar,color='b',label='VolEff')
...
for rpm in RPM:
    ...
    ITar[int((N-RPM[0])/100)] = IT
    mair[int((N-RPM[0])/100)] = mass_of_air_inducted
plt.plot(RPM,IHPar,color='k',label='FDIntake')
plt.grid()
plt.legend()
plt.xlabel('RPM')
plt.ylabel('Indicated power (HP)')
# plt.show()
plt.savefig('graphs/PowerCurves/VolEffvsIntake_power')

# fig,ax = plt.subplots()
# ax.plot(RPM,IHPar,color='b')
# ax.set_xlabel('RPM')
# ax.set_ylabel('Indicated Horsepower',color='b')
# ax2=ax.twinx()
# ax2.plot(RPM, ITar,color='r')
# ax2.set_ylabel('Indicated Torque',color='r')
# plt.savefig('graphs/PowerCurves/FDIntakePower&Torque')
